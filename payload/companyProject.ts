          ...
          descriptions: [
            ...
            },
            {
              ...
              descriptions: [
                ...
                {
                  content:
                    'JS에서 문자열/데이터 수집 → 데이터 변환 → Linear Memory 배치 → 포인터·길이 전달 → WASM 처리 → 포인터/버퍼 수거·해제의 과정을 문서화. 엔진/프론트 간 커뮤니케이션 프로토콜(입출력 규약, 메모리 소유권 및 해제 책임(alloc/free), 에러 코드)을 명문화하여 가이드라인 수립',
                },
                {
                  content:
