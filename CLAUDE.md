...
2. 사용한다면 이 프로젝트에 이미 설치된 `resume-guideline` 스킬(같은 폴더의 `conventions.md`·`facts.md`·`decisions.md` 포함)을 쓸지, 외부 스킬을 쓸지
...
- `commit-workflow` — 이 저장소에서 커밋을 요청받았을 때(export 전 개발 서버 확인, 푸시는 명시 요청 시에만)
- `resume-final-check` — 이력서를 배포하기 전이나 "최종 점검·검증"을 요청받았을 때(점검 스크립트 포함, 읽기 전용)
